...
def scoreCirc_commonEmitterAmp_resilenceMode(circuit, gen, indi, MOEAMODE):
    """
    # Slice the current scoreFunction into cheching the BigCircuitMatrix here
    ...
    def evaluateNetlistAppendResults(circuitFilename, results_list):
        ...
        return disfCount, _dcvout_rmse, _maxpower, _gain_stddev_norm
    ...
    score_array = np.array([0,0,0,0], dtype="float64") # Watch now: this is the real numbers of all objectives. Optimize that code.
    # [disfCount, _dcvout_rmse, _maxpower, _gain_stddev_norm]
    ...
    if OcSc > 1:    # if outer connections are short-circuited, punish the individual based on the matrix health and exit.
        ...
        if robustMode:
            ...
            disfCount, _dcvout_rmse, _maxpower, _gain_stddev_norm = evaluateNetlistAppendResults(circuit.filename, results_list)
            os.remove(circuit.filename) #cleanup current subcircuit
            score_array += np.array([disfCount, _dcvout_rmse, _maxpower, _gain_stddev_norm])
            ...
            if MOEAMODE == 0:
                score +=  sum(score_array[1:]) # disfCount (and _dcgain) are left out

                if disfCount > 0:
                    score = 0 + np.exp(disfCount) * 1e3 + random.random()*10
                
                if debug > 2:
                    print(_dcgain, _dcvout_rmse, _maxpower, _gain_stddev_norm)
