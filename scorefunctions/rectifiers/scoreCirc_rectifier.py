...
import scorefunctions.rectifiers.runme as runme

import os, sys

def scoreCirc_rectifier(circuit, gen, indi, MOEAMODE):
    """
    scoreCirc_rectifier_resilenceMode is a wrapper function which calls a runme file. 
            ...
            for elm in MODELSCHEME:
                ...
                for mod in MODELSCHEME[elm][1:]:  # Consider only anti-models. The all nominal is already evaluated. 
                    ...
                    makeNetlist_netlister(circuit, 
                                          ...
                                          ModelName = mod)
                    # ----------------------------------- #
                    score, results = runme.evaluate_rectifier(circuit.filename)
                    results_list.append(results)
                    score_array = np.append(score_array, score)
                    # ----------------------------------- # 
