...
import scorefunctions.amplifiers.runme as runme

import os, sys

def scoreCirc_commonEmitterAmp_resilenceMode(circuit, gen, indi, MOEAMODE):
    """
    scoreCirc_commonEmitterAmp_resilenceMode is a wrapper function which calls a runme file. 
    ...
    def evaluateNetlistAppendResults(circuitFilename, results_list):
        ...
        cost, results = runme.evaluate_CommonEmitterAmp(circuit.filename)
    ...
    if OcSc > 1:    # if outer connections are short-circuited, punish the individual based on the matrix health and exit.
        ...
        if robustMode:
            ...
            makeNetlist_netlister(circuit)  
            score, results = runme.evaluate_CommonEmitterAmp(circuit.filename, nominalTopology=True)
            results_list.append(results)
            ...
            for elm in MODELSCHEME:
                ...
                for mod in MODELSCHEME[elm][1:]:  # Consider only anti-models. The all nominal is already evaluated. 
                    ...
                    makeNetlist_netlister(circuit, 
                                          ...
                                          ModelName = mod)
                    # ----------------------------------- #
                    score, results = runme.evaluate_CommonEmitterAmp(circuit.filename)
                    results_list.append(results)
                    score_array = np.append(score_array, score)
                    # ----------------------------------- # 
